...
export default function SidebarAss({ isOpen, toggleSidebar }) {
  ...
  return (
    <div className="flex inset-0 h-[70%] items-center justify-center">
      ...
      <aside
        className={`fixed top-25 h-[calc(90vh-4rem)] bg-white rounded-[22px] flex flex-col justify-between transition-all duration-300 z-40 shadow-lg
        ${isOpen ? "w-1/4" : "w-20"}`}
      >
        {/* Logo + Toggle */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-semibold text-gray-800 text-base">
                    {currentUser?.username || "Association"}
                  </div>
                  <div className="text-sm text-blue-400 capitalize">Association</div>
                </div>
              </div>
            ) : (
              <div className="w-10 h-10 rounded-full bg-blue-400 flex items-center justify-center font-bold text-white text-lg">
